...
static int find_oid(const uint8_t* exts, size_t exts_size, const uint8_t* oid, size_t oid_size,
    ...
    static_assert(sizeof(sgx_quote3_t) >= 128, "need to change ASN.1 length-of-octet-string limit");
...
int cmp_crt_pk_against_quote_report_data(mbedtls_x509_crt* crt, sgx_quote3_t* quote) {
    int ret;
...
int extract_quote_and_verify_pubkey(mbedtls_x509_crt* crt, sgx_quote3_t** out_quote,
