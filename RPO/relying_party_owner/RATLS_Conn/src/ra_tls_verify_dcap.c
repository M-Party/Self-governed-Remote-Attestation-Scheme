...
int ra_tls_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    uint8_t* p_quote_collateral = parseCollateral(rpe_tcbinfo);
...
uint8_t* parseCollateral(const char* base64_encoded_collateral) {
    // Parse collateral
    json_t* collateral_object = json_loads(base64_encoded_collateral, 0, NULL);
