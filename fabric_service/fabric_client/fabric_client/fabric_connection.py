...
class Connector:
    ...
    def get_quote_by_ids(self, worker_ids):
        while True:
            try:
                return self._get_quote_by_ids(worker_ids)
            except Exception as e:
                logger.error("error when get quotes from Fabric "
                             "block chain. error message: %s. Retry "
                             "after 10 seconds." % e)
                time.sleep(10)
    ...
    def upload_customer_enclaves(self, customer_enclaves):
        while True:
            try:
                return self._upload_customer_enclaves(customer_enclaves)
            except Exception as e:
                logger.error("error when uploading customer enclaves to Fabric "
                             "block chain. error message: %s. Retry "
                             "after 10 seconds." % e)
                time.sleep(10)
    ...
    def get_customer_enclaves(self, job_ids):
        while True:
            try:
                return self._get_customer_enclaves(job_ids)
            except Exception as e:
                logger.error("error when getting customer enclaves from Fabric "
                             "block chain. error message: %s. Retry "
                             "after 10 seconds." % e)
                time.sleep(10)
