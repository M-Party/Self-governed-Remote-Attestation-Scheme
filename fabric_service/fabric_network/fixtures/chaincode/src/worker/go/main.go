...
func (t *WorkerRegistry) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "getQuoteByIds" {
		return t.getQuoteByIds(stub, args)
	...
	} else if function == "uploadCustomerEnclaves" {
		return t.uploadCustomerEnclaves(stub, args)
	} else if function == "getCustomerEnclaves" {
		return t.getCustomerEnclaves(stub, args)
...
// getQuoteByIds - This function xxx
func (t *WorkerRegistry) getQuoteByIds(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	logger.Info("getQuoteByIds")
	if len(args) != 1 {
		logger.Errorf("Expected parameter is 1, received %d", len(args))
		return shim.Error("getQuote must include 1 argument, workerIDs")
	}

	// Get the RPE IDs from param
	rpeIds := strings.Split(args[0], ",")

	var workerQuotesResParam WorkerQuotesResParam
	QuoteMap := make(map[string]string)
	for _, rpeId := range rpeIds {
		// Get quote for the worker
		quoteBytes, err := stub.GetState("QUOTE_" + rpeId)
		if err != nil {
			return shim.Error(err.Error())
		}
		if quoteBytes == nil {
		...
		QuoteMap[rpeId] = quote
	}

	workerQuotesResParam.Quotes = QuoteMap
	workerQuotesResParamBytes, err := json.Marshal(workerQuotesResParam)
	if err != nil {
		return shim.Error(err.Error())
	}
	return shim.Success(workerQuotesResParamBytes)
}
...
// uploadCustomerEnclaves - This function xxx
func (t *WorkerRegistry) uploadCustomerEnclaves(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	logger.Info("uploadCustomerEnclaves")
	if len(args) != 1 {
		logger.Errorf("Expected parameter is 1, received %d", len(args))
		return shim.Error("uploadCustomerEnclaves must include 1 argument, customerEnclaves")
	}

	customerEnclaves := make(map[string]string)
	err := json.Unmarshal([]byte(args[0]), &customerEnclaves)
	if err != nil {
		return shim.Error(err.Error())
	}

	for jobId, ceInfo := range customerEnclaves {
		ceInfoBytes, err := json.Marshal(ceInfo)
		if err != nil {
			return shim.Error(err.Error())
		}
		err = stub.PutState("CUSTOMER_ENCLAVES_"+jobId, ceInfoBytes)
		if err != nil {
			return shim.Error(err.Error())
		}
	}
...
func (t *WorkerRegistry) getCustomerEnclaves(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	logger.Info("getCustomerEnclaves")
	if len(args) != 1 {
		logger.Errorf("Expected parameter is 1, received %d", len(args))
		return shim.Error("getCustomerEnclaves must include 1 argument, jobIDs")
	}

	// Get the jobIDs from param
	jobIDs := strings.Split(args[0], ",")

	// Get customerEnclaves
	customerEnclaves := make(map[string]string)
	for _, jobID := range jobIDs {
		ceInfoBytes, err := stub.GetState("CUSTOMER_ENCLAVES_" + jobID)
		if err != nil {
			return shim.Error(err.Error())
		}
		if ceInfoBytes == nil {
			return shim.Error("The ceInfo of job " + jobID + " has not been uploaded")
		}
		var ceInfo string
		err = json.Unmarshal(ceInfoBytes, &ceInfo)
		if err != nil {
			return shim.Error(err.Error())
		}
		customerEnclaves[jobID] = ceInfo
	}

	var workerCustomerEnclaves WorkerCustomerEnclaves
	workerCustomerEnclaves.CustomerEnclaves = customerEnclaves
	workerCustomerEnclavesBytes, err := json.Marshal(workerCustomerEnclaves)
	if err != nil {
		return shim.Error(err.Error())
	}

	return shim.Success(workerCustomerEnclavesBytes)
}
