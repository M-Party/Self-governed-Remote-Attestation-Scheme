...
class RATLS:
    ...
    def getData(self):
        RAtls.get_ce_data.argtypes = []
        RAtls.get_ce_data.restype = ctypes.c_char_p
        result = RAtls.get_ce_data()
        return ctypes.string_at(result).decode()
        
    
    def something(self, data):
        RAtls.wait.argtypes = [ctypes.c_char_p]
        RAtls.wait.restype = ctypes.c_int
        b_data = data.encode('utf-8')

        result = RAtls.wait(ctypes.c_char_p(b_data))
        return result      